# Log Analyzer – SM-W Lise+ (IE Tecnologia)
...
This repository provides a Python-based system for automated analysis of log files generated by the **SM-W Lise+** electrical monitoring device (IE Tecnologia), focused on Phase A measurements.

The tool performs:

- 📈 Time-series plots for RMS Voltage (V), RMS Current (A) and Active Power (W)  
- ⚡ Daily energy usage calculation (kWh/day) based on `epa_c`  
- 🚨 Outlier detection using the IQR method (voltage, current, power)  
- 📝 Automatic reporting with statistics, kWh/day and outliers  
- 💾 Interactive menu to select the default `./logs` folder or any custom path  
- 📑 Report preview in the terminal, save to `.txt` file or both  
...
└── logs/            # (optional) Default folder for .txt log files
```
...
|-----------|-----------------------------------------------|------|
| **hour**  | Hour of the measurement (0–23)                | h    |
| **minute**| Minute of the measurement (0–59)              | min  |
| **second**| Second of the measurement (0–59)              | s    |
...
The SM-W Lise+ exports some fields **scaled by ×100**:
...
If the SM-W Lise+ is configured with a different scale, adjust the factor in the `calcular_kwh_por_dia` function accordingly.
